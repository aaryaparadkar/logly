...
export type ChangeType = 'feature' | 'fix' | 'improvement' | 'breaking' | 'docs' | 'chore';
...
export class AiService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.model = this.configService.get<string>('AI_MODEL') || 'mistral-small-latest';
  }
...
${commitList}`;
  ...
  async categorizeCommits(commits: GithubCommit[]): Promise<CategorizedCommit[]> {
    if (!this.apiKey) {
  ...
  private parseResponse(response: string, commits: GithubCommit[]): CategorizedCommit[] {
    ...
    for (const line of lines) {
      const match = line.match(/^(FEATURE|FIX|IMPROVEMENT|BREAKING|DOCS|CHORE):\s*(\S+)\s*-\s*(.+)$/i);
      if (match) {
        const [, typeStr, shortSha, title] = match;
    ...
    if (results.length === 0) {
      return commits.map(commit => {
        ...
        if (msg.includes('feat') || msg.includes('feature') || msg.includes('add')) type = 'feature';
        else if (msg.includes('fix') || msg.includes('bug') || msg.includes('fix')) type = 'fix';
        else if (msg.includes('improve') || msg.includes('enhance') || msg.includes('optimize')) type = 'improvement';
        else if (msg.includes('break') || msg.includes('migration')) type = 'breaking';
...
Example: "Added dark mode support|Implemented system theme detection"`
    ...
    try {
      const response = await axios.post(
        'https://api.mistral.ai/v1/chat/completions',
        {
          model: this.model,
          messages: [
            { role: 'system', content: 'You are a changelog writing expert. Be concise and clear.' },
            { role: 'user', content: prompt },
          ],
          temperature: 0.5,
          max_tokens: 200,
